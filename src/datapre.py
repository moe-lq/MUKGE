...
class Data(object):
    ...
    # 预处理数据，把对称关系的权重差拉开
    def pre_load_data(self, filename, splitter='\t', line_end='\n'):
        triples = []
        ...
        for line in open(filename):
            ...
            key = h + r + t
            reverse_key = t + r + h  # 拼接对称关系
            if self.temp_map.get(reverse_key) is not None:  # 如果存在对称关系，就进行后续判断
                if abs(w-self.temp_map.get(reverse_key)) > 0.6:
                    triples.append([h, r, t, w])
                else:
                    if w > self.temp_map.get(reverse_key):  # 如果W大，则W增加一定权重，减少对称关系的权重
                        w = w + (1 - w) / 2

                    else:
                        w = w / 5
                        # if [t, r, h, temp_map.get(reverse_key)] in triples:
                        #     triples.remove([t, r, h, temp_map.get(reverse_key)])
                        # triples.append([t, r, h, temp_map.get(reverse_key) + (1 - temp_map.get(reverse_key) % 2)])
        ...
        with open(filename, 'w', newline='') as f:
            ...
            tsv_w.writerows(np.array(triples))
...
class BatchLoader():
    ...
    def gen_psl_samples(self):
        ...
        soft_h, soft_r, soft_t, soft_lb = samples[:, 0].astype(int), samples[:, 1].astype(int), samples[:, 2].astype(
            int), samples[:, 3]
